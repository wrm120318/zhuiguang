...
> 中学校园师生学科学习共享站 —— 美文共赏、资料共享、**学科论坛**、题库自测、单题训练、成绩查询、站内信、经验值系统，一站式校园学科学习社区。
>
...
| 开源协议 | MIT |
| 当前版本 | v4.2.9（上传失败修复：彻底移除 v4.2.7 残留 Service Worker 物理文件 + 强化 main.ts unregister） |
...
| 公告与指南 | 站点公告、使用指南页面，**支持 CommonMark + KaTeX + 视频/PDF 嵌入** 富文本，**超管可多次编辑**，支持置顶与全站/班级范围 | 超管发布、全员查看 |
| 主题切换 | 多主题配色，后台可配置主题参数 | 超管配置、全员使用 |
| 功能开关 | 后台可视化管理自助注册等功能开关 | 超管 |
| 用户管理 | 用户增删改查、重置密码、启用/禁用账号、角色分配 | 超管 |
| 通知中心 | 收到评论/点赞自动入通知中心，桌面右上角弹窗（蓝色=评论、绿色=点赞），点击跳到对应评论锚点，15s 轮询 + 增量检测 | 全部角色 |
| 学科论坛 | 每学科独立板块，话题标签 + 免审阈值 + 教师代审 + 二级评论；编辑器与全站统一（MarkdownEditor） | 全部角色 |
| 通用 Markdown 编辑器 | `<MarkdownEditor>` 公共组件：CommonMark + GFM + KaTeX + 提及/高亮/视频/PDF/B站/file 附件 + HTML 子集，工具栏 + 实时预览 + 拖拽粘贴上传 + 撤销/重做 | 全部角色 |
...
- **v3.0.0 墨金学术主题上线**：经典暖橘 + 墨金学术（浅/深双档），超级管理员后台可切换、全站全用户生效
- **全站液态玻璃质感 v7**：对标大厂真柔光玻璃（几乎透明 L1/L2/L3 + 明显亮边 + 多层柔影只浮不框），自托管衬线字体（Noto Serif SC）+ 主题背景图（SVG 程序化）
- Bug14~17 修复：输入框金色直角框、弹窗灰色矩形条、Hero 矩形边框、墨金深档特异性（详见 CHANGELOG v3.0.0）
- **v3.0.10 通知中心 direction 枚举值修复**：el-drawer direction 改用 ltr/rtl 替代 right/left，全局 drawer position 强制
- **v4.0.0 15+ 业务 bug 集中修复**：canManageSubject 异步化、requireSubjectStaff 中间件、考试题源兼容模式、学科榜从 exp_logs 聚合、博客 PATCH/编辑入口、通知 read-all
...
| v4.2.9 | 上传失败修复：物理删除 `public/sw-download.js`（避免被 fetch 重新激活）+ 强化 `main.ts` 的 `cleanupLegacySW`：await 全部 `unregister()` 真正完成。v4.2.7 残留 SW 是用户上传"稍大一些的文件"失败的真正原因（curl 实测 Supabase 50KB/1MB/5MB 全部 200 OK） | 已上线（当前） |
| v4.2.8 | 紧急回退：v4.2.7 的 SW 拦截方案失败（CF Pages SPA fallback 把 `/api/download/*` 截到 index.html，导致下载 HTML 文件），回退到 v4.2.6 fetch+blob 稳定方案；`main.ts` 加 `serviceWorker.unregister()` + `caches.delete()` 清理残留旧 SW | 已上线 |
...
### 11. 经典 :root 像素级不动（v3.0.0 新增）

经典暖橘主题自 v1.0 沿用，是铁律级基线。**禁止**修改 `src/styles/main.css` 中经典档 `:root` 的任何变量/样式。

### 12. 新视觉仅限墨金作用域（v3.0.0 新增）

所有墨金主题新样式只能写在 `.zg-inkgold` / `.zg-inkgold-dark` 选择器下。**禁止**把墨金专属样式写成全局 scoped 强加进经典（"一锅端"是高频翻车点）。

### 13. 墨金深浅两档 CSS 特异性（v3.0.0 新增，Bug17）

- 浅档：`html.zg-inkgold`（特异性 0,1,1）
- 深档：`html.zg-inkgold.zg-inkgold-dark`（0,2,1）
- 提亮档：`html.zg-inkgold.zg-inkgold-dark.zg-inkgold-bright`（0,3,1）
- **禁止**让深档选择器特异性低于浅档，否则深档变量永不生效（曾致深底深字不可读）。

### 14. 液态玻璃材质规则（v3.0.0 新增）

- 墨金玻璃三级：L1 0.08 / L2 0.10 / L3 0.12（几乎透明）+ 明显亮边 0.6~0.7 + blur 10~12px
- 多层柔影只"浮"不"框"；静态面板**禁止**用 `--zg-rim`（`inset 0 0 0 1px` 发丝金边 = 矩形界限根因）
- 背景图（`public/bg/inkgold-paper.svg` + `-dark.svg`）必须真正透出
- **禁止**把玻璃压到 0.35 以上做成"白瓷不透光"，**禁止**用 `box-shadow: inset 0 1px 0` 顶部釉光（像矩形边框，Bug16）

### 15. 输入框圆角与主题圆角解耦（v3.0.0 新增，Bug14）

主题圆角 `--zg-radius` 只给卡片/按钮/弹窗；`input/textarea/select` 固定 10px `!important`。`:focus-visible` 必须排除 `.el-input__wrapper / .el-textarea__inner / .el-select__wrapper / .el-input__inner`，否则出现金色直角矩形边框。
