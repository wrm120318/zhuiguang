...
- 后端（Hono 框架，136+ 路由）部署到 Cloudflare Workers → https://api.xkzg.dpdns.org
- 数据库：Cloudflare D1（SQLite 兼容，23 张表）
...
✅ 第二件：检查服务当前状态，确保能跑通
      → 执行以下命令，确认都返回 HTTP 200：
         ...
         2. curl -s -o /dev/null -w "HTTP: %{http_code}\n" https://api.xkzg.dpdns.org/__zg_health
...
【铁律15】输入框圆角与主题圆角解耦（v3.0 新增）
   ...
   ✅ :focus-visible 必须排除 .el-input__wrapper / .el-textarea__inner / .el-select__wrapper / .el-input__inner
...
- 当前版本：v3.0.0（经典暖橘 + 墨金学术双主题，全站液态玻璃 v7）
