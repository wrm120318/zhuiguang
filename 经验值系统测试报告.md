...
**建议修复：**
在删除美文时，应该同时修正 `likes` 计数：
```typescript
await run('UPDATE articles SET likes = MAX(0, likes - ?) WHERE id=?', likeCount, id)
```
...
### 删除美文逻辑（worker-api.ts:1433-1454）
```typescript
...
需要改进的问题：
- ⚠️ 点赞数与实际点赞状态不一致（需要修复）
- ⚠️ 经验值记录保留策略（当前实现合理，保持即可）
