...
<template>
  <div class="page zg-container home-page">
...
/* ============================================================
   墨金学术 · v17.1 真液态玻璃设计语言
   核心：中blur(14-16px)+暖金细边框+柔和外阴影+内顶高光（无内底暗边！）
   ============================================================ */
/* L1 Hero 区：主角级液态玻璃 */
.zg-inkgold .hero {
  ...
  transition: transform .45s var(--zg-spring), box-shadow .4s var(--zg-ease);
}
.zg-inkgold .hero:hover {
  transform: translateY(-4px) scale(1.005);
  box-shadow: 0 12px 36px rgba(120,90,30,0.10), inset 0 1px 0 rgba(255,255,255,0.6) !important;
}
/* Hero顶部1px细高光条（在金色发光条之上，额外的玻璃质感）
   注意：不覆盖.hero::before金色发光品牌条，而是用额外层 */
.zg-inkgold .hero-glaze {
  ...
  background: radial-gradient(circle at 50% 50%, rgba(var(--zg-accent-rgb), .06), rgba(var(--zg-primary-rgb), .02) 40%, transparent 70%);
  filter: blur(100px);
  opacity: 0.7;
}
/* Hero额外的顶部玻璃高光（不影响金色品牌光线） */
.zg-inkgold .hero > .hero-grid { position: relative; z-index: 2; }
/* 统计条容器：整体L1玻璃，内部分隔 */
.zg-inkgold .hero-stats {
  background: linear-gradient(155deg, rgba(255,255,255,0.50), rgba(255,253,248,0.45)) !important;
  border: 1px solid rgba(184,148,63,0.08) !important;
  box-shadow: 0 2px 12px rgba(120,90,30,0.04), inset 0 1px 0 rgba(255,255,255,0.4) !important;
  -webkit-backdrop-filter: blur(10px) saturate(var(--zg-glass-sat));
  backdrop-filter: blur(10px) saturate(var(--zg-glass-sat));
  padding: 18px 24px !important;
  gap: 0;
}
/* 数据卡：不要单独玻璃，在stats容器内 */
.zg-inkgold .hs-item {
...
.zg-inkgold .hs-item::before { display: none !important; }
...
.zg-inkgold .cta-primary {
  ...
  box-shadow: 0 4px 14px rgba(var(--zg-primary-rgb), .18), inset 0 1px 0 rgba(255,255,255,.25) !important;
  color: #fff !important;
  border: none !important;
  transition: all .3s var(--zg-spring) !important;
}
.zg-inkgold .cta-primary:hover {
  box-shadow: 0 6px 20px rgba(var(--zg-primary-rgb), .24), inset 0 1px 0 rgba(255,255,255,.3) !important;
...
.zg-inkgold .cta-ghost {
  ...
  position: relative;
  overflow: hidden;
  transition: transform .3s var(--zg-spring), box-shadow .3s var(--zg-ease), background .3s var(--zg-ease);
}
.zg-inkgold .cta-ghost:hover {
  box-shadow: 0 8px 24px rgba(120,90,30,0.08), inset 0 1px 0 rgba(255,255,255,0.5) !important;
...
.zg-inkgold .subj-chip {
  ...
  position: relative;
  overflow: hidden;
  transition: transform .35s var(--zg-spring), box-shadow .35s var(--zg-ease);
}
/* 公告栏 */
.zg-inkgold .announce-bar {
  background: linear-gradient(90deg, rgba(var(--zg-primary-rgb), .06), rgba(var(--zg-accent-rgb), .02) 62%, transparent) !important;
  border: none !important;
  border-left: 3px solid rgba(var(--zg-primary-rgb), .40) !important;
  box-shadow: none !important;
...
.zg-inkgold .subj-chip:hover {
  ...
  box-shadow:
    0 8px 24px rgba(120,90,30,0.09),
...
.zg-inkgold.zg-inkgold-dark .announce-bar {
  ...
  border-left-color: rgba(var(--zg-primary-rgb), .45) !important;
}
...
.art-card { overflow: hidden; cursor: pointer; border-radius: 22px; transition: transform .3s cubic-bezier(.22,1,.36,1), box-shadow .3s; padding: 0 !important; }
.art-card:hover { transform: translateY(-4px); }
.ac-cover { position: relative; width: 100%; height: 180px; background-size: cover !important; background-position: center !important; background-repeat: no-repeat !important; margin: 0 !important; border-radius: 0 !important; }
...
@media (max-width: 768px) {
  ...
  /* 墨金移动端：hero 圆角24px玻璃 */
  .zg-inkgold .hero { padding: 20px 16px 18px !important; border-radius: 22px !important; margin: 24px 0 14px !important; min-height: auto !important; }
...
@media (max-width: 480px) {
  ...
  .quick-row { gap: 8px; }
  .zg-inkgold .hero { padding: 18px 14px 16px !important; border-radius: 20px !important; margin: 20px 0 12px !important; min-height: auto !important; }
