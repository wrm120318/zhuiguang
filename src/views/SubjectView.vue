...
const tabs = computed(() => {
  ...
  // 【v4.1.1 学科论坛】由 modules.forum 控制（默认 true）
  if (m.forum !== false) t.push({ key: 'forum', label: '学科论坛' })
...
const moduleLabels: Record<string, string> = {
  announcement: '公告栏', resources: '资料共享', articles: '美文共赏',
  forum: '学科论坛', query: '数据查询', quiz: '题库自测', leaderboard: '学科榜'
}

// 【v4.1.1】论坛子开关：<1000 字免审阈值
const forumThresh = ref(0)
async function saveForumThresh() {
  if (!subject.value) return
  await api.updateSubjectForumConfig(subject.value.id, { forumAutoApproveThreshold: Number(forumThresh.value) || 0 })
  ElMessage.success('论坛免审阈值已更新')
  subject.value.forum_auto_approve_threshold = Number(forumThresh.value) || 0
}
watch(subject, (s) => { if (s) forumThresh.value = Number(s.forum_auto_approve_threshold || 0) }, { immediate: true })
...
async function downloadResource(r: any) {
  ...
  // 【v4.2.8 紧急回退】v4.2.7 的 SW 拦截方案失败：
  //   /api/download/* 在 SW 激活前会被 CF Pages SPA fallback 截到 index.html，
  //   导致用户下载到 HTML 页面。回退到 v4.2.6 fetch+blob 稳定路径。
  try {
...
async function saveAnnounce() {
  if (!subject.value) return
  // 【v4.0.1 学科公告】教师/超管：走专门的 announcement 端点（后端 requireSubjectStaff）
  await api.updateSubjectAnnouncement(subject.value.id, announceDraft.value)
...
<template>
  <div class="page zg-container" v-if="subject">
      ...
      <div class="sh-manage" v-if="canManage">
        <el-popover trigger="click" width="260" placement="bottom-end">
          ...
          <div class="mod-toggle">
            ...
            <!-- 【v4.1.1】论坛子开关：<1000 字免审 -->
            <div v-if="subject.modules.forum !== false" class="mt-row mt-sub">
              <span>论坛 < 1000 字免审</span>
              <el-input-number v-model="forumThresh" :min="0" :step="100" :max="10000" size="small" controls-position="right" style="width:120px" @change="saveForumThresh" />
            </div>
            <div v-if="subject.modules.forum !== false && forumThresh > 0" class="mt-hint">
              当前阈值 {{ forumThresh }} 字（学生/跨学科教师发帖纯文本 ≤ 阈值将自动发布）
            </div>
          </div>
        </el-popover>
      </div>
    ...
    <div class="sub-nav glass">
      <div v-for="t in tabs" :key="t.key" class="sub-tab" :class="{ on: activeTab === t.key }" @click="onTabClick(t.key)">{{ t.label }}</div>
    </div>
...
.sub-tab:hover { color:var(--zg-text); background:rgba(var(--zg-primary-rgb),.06); }
...
.rc-tag { font-size:11px; color:var(--zg-accent); background:rgba(var(--zg-primary-rgb),.12); padding:2px 8px; border-radius:6px; }
.rc-foot { display:flex; justify-content:space-between; align-items:center; font-size:12px; color:var(--zg-text-dim); padding-top:10px; border-top:1px solid rgba(var(--zg-primary-rgb),.06); }
...
.rank-item.top { background:rgba(var(--zg-primary-rgb),.06); }
.rank-no { width:26px; height:26px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:13px; background:rgba(var(--zg-primary-rgb),.1); }
.rank-no.n1 { background:linear-gradient(135deg,var(--zg-accent),var(--zg-primary)); color:var(--zg-text); }
...
.mt-row.mt-sub { padding-left:10px; border-top:1px dashed #eee; margin-top:6px; padding-top:10px; }
.mt-hint { font-size:12px; color:#94A3B8; line-height:1.5; padding:4px 0; }

/* 桌面端适配：subject 子站内容居中 */
@media (min-width: 1024px) {
  .zg-container { max-width: 1200px; margin: 0 auto; }
  .subj-tabs { max-width: 1200px; margin: 0 auto; }
}
