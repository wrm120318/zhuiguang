...
- [Q11. 网站有几种主题？怎么切换？](#q11-网站有几种主题怎么切换)
- [Q12. 墨金主题下界面是全透明玻璃，文字看不清/有矩形框怎么办？](#q12-墨金主题下界面是全透明玻璃文字看不清有矩形框怎么办)
- [Q13. 墨金主题字体/背景图是 CDN 加载的吗？](#q13-墨金主题字体背景图是-cdn-加载的吗)
...
当前共 **3 种视觉主题**，由超级管理员在后台统一切换，全站全用户生效：

- **经典暖橘**（默认，自 v1.0 沿用，像素级不动）
- **墨金学术·浅色**（暖米白 #FAF8F4 + 沉稳金 #BA7517）
- **墨金学术·深色**（温润暖黑 #1B1710）

切换路径：超管登录 → 后台 → 主题设置 → 选择 `designMode`（classic/inkgold）+ `inkgoldTone`（light/dark）→ 保存并发布。前端 `src/store/theme.ts` 的 `applyTheme()` 据此给 `<html>` 加 `.zg-inkgold` / `.zg-inkgold-dark` 类。

## Q12. 墨金主题下界面是"全透明玻璃"，文字看不清/有矩形框怎么办？

这是 v3.0.0 已修复的问题，正常状态不应出现。若你改样式后出现，检查三项铁律：

1. **玻璃三级材质**：L1 0.08 / L2 0.10 / L3 0.12（几乎透明）+ **明显亮边 0.6~0.7** + blur 10~12px。只透不亮 = "全透明垃圾塑料"（Bug 反馈原话）。
2. **禁止 `--zg-rim`**：静态面板的 `inset 0 0 0 1px` 发丝金边会让区块显出矩形界限，必须移除。
3. **输入框金色直角框**：`:focus-visible` 必须排除 `.el-input__wrapper` 等输入框类（Bug14），否则 outline 不跟随圆角出现直角矩形。

## Q13. 墨金主题字体/背景图是 CDN 加载的吗？会不会依赖外部？

**零外部依赖**。自托管方案：

- 衬线字体：`public/fonts/noto-serif-sc-{600,700,800}.woff2`（Noto Serif SC 简体中文，Vite 原样拷到 dist/fonts/），`src/styles/main.css` 顶部 @font-face 引用。
- 主题背景图：`public/bg/inkgold-paper.svg`（浅）/ `inkgold-paper-dark.svg`（深），SVG 程序化纹理（网格 + 光斑 + 斜向丝光），玻璃透出其质感。

---

## Q14. 美文/帖子的封面是空白的，怎么办？

**根因**：封面是外链图片。早期用 `picsum.photos` 等境外图床，这些域名**在国内不可达** → 全部空白。

**现状（v4.4.2 起）**：封面改用 **Bing 每日壁纸美图池**（国内稳定可达、高清、免费、零 B2 调用）。
- 新建/编辑美文时，封面留空会自动分配一张美图；也可点「🖼️ 换一张美图」换图。
- 若某张外链图仍加载失败，全局兜底会自动换成 Bing 美图，不会再出现破图/空白。
- **其他封面**（头像、公告、页面封面、资源）走原来的「上传」逻辑（存 B2、经 `/api/file/` 返回），不受影响。

**排查**：封面空白先确认是不是自己填了某个境外图床 URL；换成 Bing 美图或点「换一张」即可。

## Q15. 后台监控的「回源次数」和 B2 控制台数字对不上，为什么？

**这是正常的，不是 bug。**

- B2 **免费账户不暴露官方交易计数 API**（实测响应头没有 `b2-api-*-transaction-count-today`，`b2_get_account_info` 也 404）。
- 面板里的「回源次数 / 本地统计·非官方」是本系统**自己发出的回源计数**，只统计经本 Worker 的下载，不可能等于 B2 控制台（控制台统计的是账号全部流量 + 历史）。

**正确做法**：后台「B2 交易与配额」tab 里有「📊 官方每日实耗核对」卡片，把 B2 控制台「数据限度」页的当日数字（B 类 / C 类 / 存储 MB / 下载 MB）**手动录入**，
面板会据此展示「官方已用 / 上限」进度条，这才是对齐官方的口径。

**关于"免费最大化"**：修复后 C 类已压到 ≈1 次/日（token 落 D1 全球共享）；监控面板零 B2 调用（只读 D1 快照）；盘点（Class A）每日限 1 次。
你之前看到的 C=27 / B=8 是修复前的测试/迁移消耗。
...
**学生**：进入任一学科页面 → 切换到「题库自测」tab → 点击页面顶部的「📝 我的训练记录」按钮，可分页查看所有训练提交（含得分、教师评语、提交时间），并可删除记录。

**教师/超管**：进入学科管理页的题库自测 tab，点击任意一道题目的「📊 统计」按钮，可查看全班该题的：正确率、总提交数、待批改数、各学生作答详情（含分数/评语），可删除任意学生的记录。

### Q. 单题训练和题库自测的经验值有什么区别？

| 行为 | 经验值 | action_type |
|---|---|---|
| 单题训练批改完成 | +5 | `practice_pass` |
| 题库自测通过/批改完成 | +10 | `quiz_pass` |
| 单题训练答对（即时反馈） | 由经验规则配置 | `practice_correct` |
